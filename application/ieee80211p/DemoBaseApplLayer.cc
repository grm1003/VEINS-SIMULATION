...
void DemoBaseApplLayer::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        sendReduceEvt = new cMessage("redution evt", SEND_REDUCE_EVT);
...
void DemoBaseApplLayer::populateWSM(BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial)
{
    ...
    else if (SpeedLimitation* slm = dynamic_cast<SpeedLimitation*>(wsm)) {
        slm->setSpeedLimit(25);
        slm->setSpeedbumpId(randomId);
        slm->setChannelNumber(static_cast<int>(Channel::cch));
        wsm->setUserPriority(beaconUserPriority);
    }
    else {
        if (dataOnSch)
...
void DemoBaseApplLayer::setSpeed(BaseFrame1609_4* wsm)
{
    ...
    if (wsm->getKind() == SEND_REDUCE_EVT) {
        ...
        if (slm != nullptr) {
            int speed = slm->getSpeedLimit();
            double doubleSpeed = static_cast<double>(speed);
            EV_INFO << "Velocidade recebida: " << doubleSpeed << endl;
